    ...
    value: function initMermaid() {
      ...
      this._mermaidOnSwitchTheme = this._mermaidOnSwitchTheme || function () {
        ...
        if ($mermaidElements.length) {
          // Use config from window.config if available, otherwise use defaults
          var mermaidConfig = window.config && window.config.mermaid ? 
            Object.assign({}, window.config.mermaid, {
              startOnLoad: false,
              theme: _this6.isDark ? 'dark' : (window.config.mermaid.theme || 'neutral')
            }) : {
              startOnLoad: false,
              theme: _this6.isDark ? 'dark' : 'neutral',
              securityLevel: 'loose',
              flowchart: { useMaxWidth: true },
              quadrant: { useMaxWidth: true }
            };
          mermaid.initialize(mermaidConfig);
          Util.forEach($mermaidElements, function ($mermaid) {
            // Get mermaid data from script tag
            var dataScript = document.querySelector('script.mermaid-data[data-id="' + $mermaid.id + '"]');
            var mermaidData = dataScript ? dataScript.textContent.trim() : _this6.data && _this6.data[$mermaid.id];
            if (mermaidData) {
              mermaid.render('mermaid-svg-' + $mermaid.id, mermaidData).then(function (_ref9) {
                var svg = _ref9.svg;
